...
public class SimpleLinkedList<T> implements Iterable<T> {
    ...
    // O(n^2)
    public SimpleLinkedList<T> Shuffle() throws SimpleLinkedListException {
        SimpleLinkedList<T> randomLinkedList = new SimpleLinkedList<T>();
        for(int i = 0; i < size; i++){
            double a = Math.random();
            if(a>=0.5){
                randomLinkedList.addFirst(get(i));
            } else {
                randomLinkedList.addLast(get(i));
            }
        }
